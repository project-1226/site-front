    ...
    if (cnt === 0) {
      ...
      if (window.confirm(`${cnt}개 상품을 삭제할까요?`)) {
        ...
        for (const item of list) {
          if (item.checked) {
            await axios.post("/admin/deleteProduct", {
    ...
    <div className='Product_List'>
      ...
      <div>
        ...
        <div className='mt-4'>상품수: {total}
          ...
          <input type='checkbox' onChange={(e) => onChangeAll(e)} checked={list.length === cnt} />
        </div>
        <div className='table my-2'>
          {list.map((p) =>
            <div key={p.productid} style={{ display: 'inline-block', margin: '0 10px 0px 10px' }}>
              <Card className='text-center' 
                style={{ width: '200px', height: '320px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <input type='checkbox' checked={p.checked} onChange={(e) => onCheckSingle(e, p.productid)} 
                  style={{ alignSelf: 'flex-start', marginLeft: '10px', marginTop: '10px' }} />
