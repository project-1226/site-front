...
const HealthcareModal = ({ show, handleClose, selectedCategory }) => {
  ...
  const columns = [
    ...
    { field: 'name', headerName: 'Name', width: 200 },
    {
      ...
      width: 150,
      renderCell: (params) => (
        <Button variant="contained" color="primary" onClick={(e) => handleYoutubeClick(e, params.row)}>
          운동영상 보러가기
        </Button>
      ),
    },
    {
      ...
      renderCell: (params) => (
        params.row.ai == 1 &&
        <Button variant="contained" color="primary" onClick={(e) => handleAIClick(e, params.row)}>
          AI와 운동하기
        </Button>
      ),
    },
  ...
  const getMyExercise = async () => {
    const res = await axios(`/exercise/myexercises?userid=${sessionStorage.getItem("userid")}&categoryid=${selectedCategory.categoryid}`)
  ...
  const handleSaveClick = async () => {
    ...
    if (selectedRows.length) {
      const res = await axios.post("/exercise/insert/my_exercise", selectedRows);
      console.log(res.data)
      if (res.data > 0) {
        setSelectState(`${selectedRows.length - res.data}개는 이미 들어있는 운동입니다.${selectedRows.length}개 중 ${res.data}개 저장되었습니다`)
            ...
            <div className="modal_wrap">
              <section className="healthcare_modal_btm_wrap">
                <div className='healthcare_modal_btm'>
                  {/* 글자가 모달보다 길어지면 빠져나감  */}
                  <p className='healthcare_modal_selected_title'>{selectedCategory.name}에서 선택한 운동리스트</p>
                  {selectState != "" && <p className='healthcare_modal_unselected'>{selectState}</p>}
                  {selectedRows && selectedRows.map((row) =>
                    <p className='healthcare_modal_selected_name'>☑️&nbsp; {row.name} </p>
                  )}
                </div>
              </section>
            </div>
